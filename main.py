...
def get_input():
    # Ensure the session state key exists before rendering the widget to avoid Streamlit warnings
...
# Removed GitHub analysis and token estimation utilities as GitHub integration is disabled
...
with st.sidebar:
    ...
    if model_provider == "Anthropic API":
        st.markdown(
        """
    1. Enter your Anthropic API key and chosen model below 🔑
    2. Provide details of the application that you would like to threat model  📝
    3. Generate a threat list, attack tree and/or mitigating controls for your application 🚀
    """
    )
    ...
    if model_provider == "Google AI API":
        st.markdown(
        """
    1. Enter your Google AI API key and chosen model below 🔑
    2. Provide details of the application that you would like to threat model  📝
    3. Generate a threat list, attack tree and/or mitigating controls for your application 🚀
    """
    )
    ...
    if model_provider == "Mistral API":
        st.markdown(
        """
    1. Enter your Mistral API key and chosen model below 🔑
    2. Provide details of the application that you would like to threat model  📝
    3. Generate a threat list, attack tree and/or mitigating controls for your application 🚀
    """
    )
    ...
    if model_provider == "Groq API":
        st.markdown(
        """
    1. Enter your Groq API key and chosen model below 🔑
    2. Provide details of the application that you would like to threat model  📝
    3. Generate a threat list, attack tree and/or mitigating controls for your application 🚀
    """
    )
    ...
    # GitHub and Advanced Settings removed per requirements
    ...
    st.markdown("Created By Zaidyn Security Team")
...
""")
    ...
    with col1:
            ...
            if uploaded_file is not None:
                ...
                try:
                    ...
                    if image_analysis_output and 'choices' in image_analysis_output and image_analysis_output['choices'][0]['message']['content']:
                        ...
                        # Update the logical value used by the text area
        ...
        # Add document uploader for application description (.doc, .docx)
        doc_file = st.file_uploader("Upload application description (DOC/DOCX)", type=["doc", "docx"], help="Content will populate the Describe section.")
        if doc_file is not None:
            try:
                extracted_text = ""
                file_name_lower = doc_file.name.lower()
                if file_name_lower.endswith(".docx"):
                    document = Document(doc_file)
                    extracted_text = "\n".join([para.text for para in document.paragraphs])
                else:
                    # Handle legacy .doc: convert via LibreOffice/soffice to .docx, then parse
                    with tempfile.TemporaryDirectory() as tmpdir:
                        input_path = os.path.join(tmpdir, doc_file.name)
                        with open(input_path, "wb") as f:
                            f.write(doc_file.getbuffer())

                        # Try soffice/libreoffice conversion to DOCX
                        soffice_path = shutil.which("soffice") or shutil.which("libreoffice")
                        
                        # Additional Windows-specific paths for LibreOffice
                        if not soffice_path and os.name == 'nt':  # Windows
                            possible_paths = [
                        ...
                        if not soffice_path:
                            st.error(".doc parsing requires LibreOffice (soffice). Please install it and ensure 'soffice' is on PATH, or upload a .docx file.")
                        else:
                            try:
                                ...
                                if os.path.exists(output_docx_path):
                                    ...
                                    st.error("LibreOffice conversion did not produce a .docx file. Please verify your installation or upload a .docx file.")
                                    if result.stderr:
                                        st.error(f"LibreOffice error: {result.stderr}")
                            except Exception as conv_err:
                                st.error(f".doc conversion failed via LibreOffice: {conv_err}")
                                if hasattr(conv_err, 'stderr') and conv_err.stderr:
                                    st.error(f"LibreOffice stderr: {conv_err.stderr}")

                extracted_text = extracted_text.strip()
                if extracted_text:
                    # Update the logical value used by the text area
                    st.session_state['app_input'] = extracted_text
                    st.success("Document content inserted into the description.")
            except Exception as e:
                st.error(f"Failed to extract document content: {str(e)}")

        # Use the get_input() function to render the text area bound to session state
        _ = get_input()
    ...
    with col2:
        ...
        # Create input fields for internet_facing and authentication
            ...
            # Framework selection (STRIDE or OWASP Top 10)
            framework = st.selectbox(
                label="Select threat modelling framework",
                options=["STRIDE", "OWASP Top 10"],
                key="framework",
                help="Choose STRIDE or OWASP Top 10 to guide generation."
            )
    ...
    if threat_model_submit_button and st.session_state.get('app_input'):
        ...
        # Generate the prompt based on selected framework
        if st.session_state.get('framework', 'STRIDE') == 'STRIDE':
            threat_model_prompt = create_threat_model_prompt(app_type, authentication, internet_facing, sensitive_data, app_input)
        else:
            # OWASP Top 10 prompt
            owasp_prompt = f"""
Act as a cyber security expert with more than 20 years experience of using the OWASP Top 10 to produce comprehensive application risk models.

Analyze the provided code summary, README content, and application description and identify multiple (3 or 4 where applicable) credible risks for each relevant OWASP Top 10 category (e.g., Broken Access Control, Cryptographic Failures, Injection, Insecure Design, Security Misconfiguration, Vulnerable and Outdated Components, Identification and Authentication Failures, Software and Data Integrity Failures, Security Logging and Monitoring Failures, Server-Side Request Forgery).

Your output MUST be a JSON object with keys "threat_model" and "improvement_suggestions". Under "threat_model" include an array of objects with keys "Threat Type" (set to the OWASP category), "Scenario", and "Potential Impact". Under "improvement_suggestions" include an array of strings describing what additional information would improve the next iteration.
...
{app_input}
"""
            threat_model_prompt = owasp_prompt
        ...
        with st.spinner("Analysing potential threats..."):
            ...
            while retry_count < max_retries:
                try:
                    ...
                    # Save the threat model and improvement suggestions to the session state for later use
        ...
        # Convert the threat model JSON to Markdown and persist
        ...
        # Display the threat model in Markdown for the initial generation
...
## Threat Model download controls - persistent within tab1
with tab1:
    if 'threat_model' in st.session_state and st.session_state['threat_model']:
        st.markdown("---")
        st.markdown("### Download Threat Model")
        download_format = st.selectbox(
            "Choose download format:",
            ["Markdown (.md)", "Excel (.xls)"],
            index=0,
            key="dl_format_threat_model",
            help="Select the format to download the threat model report."
        )
        if download_format == "Markdown (.md)":
            tm_md = st.session_state.get('threat_model_markdown') or json_to_markdown(
                st.session_state['threat_model'], st.session_state.get('improvement_suggestions', [])
            )
            st.download_button(
                label="Download Threat Model (.md)",
                data=tm_md,
                file_name="threat_model.md",
                mime="text/markdown",
                key="dl_threat_model_md",
            )
        else:
            # Always render the button; only buffer creation is guarded
            xls_bytes = None
            try:
                df = pd.DataFrame(st.session_state['threat_model'])
                if not df.empty:
                    output = io.BytesIO()
                    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                        df.to_excel(writer, index=False, sheet_name='ThreatModel')
                        # Apply formatting
                        worksheet = writer.sheets['ThreatModel']
                        ...
                        # Autosize columns with a max width
                        for idx, col in enumerate(df.columns):
                            max_len = max([len(str(x)) for x in df[col].astype(str).tolist()] + [len(col)])
                            worksheet.set_column(idx, idx, min(max(20, max_len), 80), wrap)
                    xls_bytes = output.getvalue()
            except Exception as e:
                st.warning(f"Excel export unavailable: {str(e)}")
            st.download_button(
                label="Download Threat Model (.xls)",
                data=xls_bytes or b"",
                file_name="threat_model.xls",
                mime="application/vnd.ms-excel",
                key="dl_threat_model_xls",
                disabled=(xls_bytes is None)
            )
        ...
        if attack_tree_submit_button and st.session_state.get('app_input'):
            ...
            with st.spinner("Generating attack tree..."):
                try:
                    ...
                    # Persist and display the generated attack tree code
                    st.session_state['attack_tree_code'] = mermaid_code
                    ...
                    # Note: Download controls are rendered persistently below

                except Exception as e:
                    st.error(f"Error generating attack tree: {e}")

        # Persistent Attack Tree download controls
        if 'attack_tree_code' in st.session_state and st.session_state['attack_tree_code']:
            st.markdown("---")
            st.markdown("### Download Attack Tree")
            attack_tree_download_format_persist = st.selectbox(
                "Choose download format:",
                ["Markdown (.md)", "Excel (.xls)"],
                index=0,
                key="dl_format_attack_tree_persist",
                help="Select the format to download the attack tree."
            )
            if attack_tree_download_format_persist == "Markdown (.md)":
                st.download_button(
                    label="Download Attack Tree (.md)",
                    data=st.session_state['attack_tree_code'],
                    file_name="attack_tree.md",
                    mime="text/plain",
                    key="dl_attack_tree_md_persist"
                )
            else:
                at_xls_bytes_persist = None
                try:
                    # Excel with Mermaid text only (no image)
                    at_out = io.BytesIO()
                    with pd.ExcelWriter(at_out, engine='xlsxwriter') as writer:
                        workbook = writer.book
                        ws = workbook.add_worksheet('AttackTree')
                        wrap_fmt = workbook.add_format({'text_wrap': True, 'valign': 'top'})
                        header_fmt = workbook.add_format({'bold': True, 'bg_color': '#F2F2F2'})
                        ws.write(0, 0, 'Mermaid Code', header_fmt)
                        mermaid_text = st.session_state['attack_tree_code']
                        ws.write(1, 0, mermaid_text, wrap_fmt)
                        ws.set_column(0, 0, 120)
                        approx_lines = max(mermaid_text.count('\n') + 1, 10)
                        ws.set_row(1, min(approx_lines * 15, 409))
                    at_xls_bytes_persist = at_out.getvalue()
                except Exception as e:
                    st.warning(f"Excel export for attack tree unavailable: {str(e)}")
                st.download_button(
                    label="Download Attack Tree (.xls)",
                    data=at_xls_bytes_persist or b"",
                    file_name="attack_tree.xls",
                    mime="application/vnd.ms-excel",
                    key="dl_attack_tree_xls_persist",
                    disabled=(at_xls_bytes_persist is None)
                )
...
""")
    ...
    if mitigations_submit_button:
        ...
        if 'threat_model' in st.session_state and st.session_state['threat_model']:
            ...
            # Generate the prompt using the create_mitigations_prompt function with framework
            mitigations_prompt = create_mitigations_prompt(threats_markdown, st.session_state.get('framework', 'STRIDE'))
            ...
            with st.spinner("Suggesting mitigations..."):
                ...
                while retry_count < max_retries:
                    try:
                        ...
                        # Display the suggested mitigations in Markdown and persist for later downloads
                        st.session_state['mitigations_markdown'] = mitigations_markdown
                        st.markdown(mitigations_markdown)

                        # Download controls rendered persistently below based on session state
            ...
            st.markdown("")
            # Removed stray header; DREAD download controls are rendered persistently within tab4 below
        else:
            st.error("Please generate a threat model first before suggesting mitigations.")

    # Persistent Mitigations download controls
    if 'mitigations_markdown' in st.session_state and st.session_state['mitigations_markdown']:
        st.markdown("---")
        st.markdown("### Download Mitigations")
        mitigations_download_format = st.selectbox(
            "Choose download format:",
            ["Markdown (.md)", "Excel (.xls)"],
            index=0,
            key="dl_format_mitigations",
            help="Select the format to download mitigations."
        )
        if mitigations_download_format == "Markdown (.md)":
            st.download_button(
                label="Download Mitigations (.md)",
                data=st.session_state['mitigations_markdown'],
                file_name="mitigation.md",
                mime="text/markdown",
                key="dl_mitigations_md",
            )
        else:
            xls_mit_bytes = None
            try:
                mm = st.session_state['mitigations_markdown']
                import re
                # Look for a markdown table with expected headers
                header_regex = r"\|\s*Threat Type\s*\|\s*Scenario\s*\|\s*Suggested Mitigation\(s\)\s*\|"
                lines = mm.split('\n')
                start_idx = -1
                ...
                if start_idx != -1:
                    ...
                    for j in range(start_idx, len(lines)):
                        if lines[j].strip().startswith('|'):
                            ...
                            break
                    # Clean out separator rows like |---|---|---|
                    cleaned = []
                    for tl in table_lines:
                        row_body = tl.strip().strip('|').strip()
                        if set(row_body.replace(' ', '').replace('-', '')) == set():
                            continue
                        cleaned.append(tl)
                    if len(cleaned) >= 2:
                        headers = [c.strip() for c in cleaned[0].strip().strip('|').split('|')]
                        rows = []
                        for row in cleaned[1:]:
                            ...
                            rows.append(cells)
                        if rows:
                            parsed_df = pd.DataFrame(rows, columns=headers)

                xout = io.BytesIO()
                with pd.ExcelWriter(xout, engine='xlsxwriter') as writer:
                    workbook = writer.book
                    worksheet = workbook.add_worksheet('Mitigations')
                    wrap = workbook.add_format({'text_wrap': True, 'valign': 'top'})
                    header_fmt = workbook.add_format({'bold': True, 'bg_color': '#F2F2F2'})
                    if parsed_df is not None and not parsed_df.empty:
                        for col_idx, col_name in enumerate(parsed_df.columns):
                            worksheet.write(0, col_idx, col_name, header_fmt)
                        for r_idx, row in parsed_df.iterrows():
                            for c_idx, col_name in enumerate(parsed_df.columns):
                                worksheet.write(r_idx + 1, c_idx, str(row[col_name]), wrap)
                        for idx, col in enumerate(parsed_df.columns):
                            max_len = max([len(str(x)) for x in parsed_df[col].astype(str).tolist()] + [len(col)])
                            worksheet.set_column(idx, idx, min(max(20, max_len), 80), wrap)
                    else:
                        worksheet.write(0, 0, 'Mitigations (Markdown)', header_fmt)
                        worksheet.write(1, 0, mm, wrap)
                        worksheet.set_column(0, 0, 120, wrap)
                xls_mit_bytes = xout.getvalue()
            except Exception as e:
                st.warning(f"Excel export for mitigations unavailable: {str(e)}")
            st.download_button(
                label="Download Mitigations (.xls)",
                data=xls_mit_bytes or b"",
                file_name="mitigation.xls",
                mime="application/vnd.ms-excel",
                key="dl_mitigations_xls",
                disabled=(xls_mit_bytes is None)
            )
...
""")
    ...
    if dread_assessment_submit_button:
        ...
        if 'threat_model' in st.session_state and st.session_state['threat_model']:
            ...
            # Display the DREAD assessment in Markdown format and persist for later downloads
            st.session_state['dread_assessment_markdown'] = dread_assessment_markdown
    ...
    # Persistent DREAD download controls
    if 'dread_assessment_markdown' in st.session_state and st.session_state['dread_assessment_markdown']:
        st.markdown("---")
        st.markdown("### Download DREAD")
        dread_download_format = st.selectbox(
            "Choose download format:",
            ["Markdown (.md)", "Excel (.xls)"],
            index=0,
            key="dl_format_dread",
            help="Select the format to download DREAD assessment."
        )
        if dread_download_format == "Markdown (.md)":
            st.download_button(
                label="Download DREAD (.md)",
                data=st.session_state['dread_assessment_markdown'],
                file_name="dread.md",
                mime="text/markdown",
                key="dl_dread_md",
            )
        else:
            xls_dread_bytes = None
            try:
                dread_df_src = st.session_state.get('dread_assessment', {})
                dread_df = pd.DataFrame(dread_df_src.get("Risk Assessment", []))
                dout = io.BytesIO()
                with pd.ExcelWriter(dout, engine='xlsxwriter') as writer:
                    dread_df.to_excel(writer, index=False, sheet_name='DREAD')
                    worksheet = writer.sheets['DREAD']
                    workbook = writer.book
                    wrap = workbook.add_format({'text_wrap': True, 'valign': 'top'})
                    header_fmt = workbook.add_format({'bold': True, 'bg_color': '#F2F2F2'})
                    for col_idx, col_name in enumerate(dread_df.columns):
                        worksheet.write(0, col_idx, col_name, header_fmt)
                    for idx, col in enumerate(dread_df.columns):
                        max_len = max([len(str(x)) for x in dread_df[col].astype(str).tolist()] + [len(col)])
                        worksheet.set_column(idx, idx, min(max(15, max_len), 80), wrap)
                xls_dread_bytes = dout.getvalue()
            except Exception as e:
                st.warning(f"Excel export for DREAD unavailable: {str(e)}")
            st.download_button(
                label="Download DREAD (.xls)",
                data=xls_dread_bytes or b"",
                file_name="dread.xls",
                mime="application/vnd.ms-excel",
                key="dl_dread_xls",
                disabled=(xls_dread_bytes is None)
            )
...
""")
            ...
            with st.spinner("Generating test cases..."):
                ...
                while retry_count < max_retries:
                    try:
                        ...
                        # Display the suggested test cases in Markdown and persist for later downloads
                        st.session_state['test_cases_markdown'] = test_cases_markdown
    ...
    # Persistent Test Cases download controls
    if 'test_cases_markdown' in st.session_state and st.session_state['test_cases_markdown']:
        st.markdown("---")
        st.markdown("### Download Test Cases")
        testcases_download_format = st.selectbox(
            "Choose download format:",
            ["Markdown (.md)", "Excel (.xls)"],
            index=0,
            key="dl_format_test_cases",
            help="Select the format to download test cases."
        )
        if testcases_download_format == "Markdown (.md)":
            st.download_button(
                label="Download Test Cases (.md)",
                data=st.session_state['test_cases_markdown'],
                file_name="test_cases.md",
                mime="text/markdown",
                key="dl_test_cases_md",
            )
        else:
            xls_tc_bytes = None
            try:
                # Parse Gherkin-style markdown into rows: Scenario, Step Type, Step
                tc_md = st.session_state['test_cases_markdown']
                lines = tc_md.split('\n')
                rows = []
                current_scenario = ""
                for raw in lines:
                    ...
                    if line.lower().startswith('scenario:'):
                        ...
                        continue
                    # Detect outlines also
                    if line.lower().startswith('scenario outline:'):
                        current_scenario = line[len('scenario outline:'):].strip()
                        continue
                    # Detect step types
                    for step_type in ['Given', 'When', 'Then', 'And', 'But']:
                        ...
                        if line.startswith(prefix):
                            rows.append({
                                ...
                                'Step Type': step_type,
                                'Step': line[len(prefix):].strip()
                            })
                            break

                # Build DataFrame if any rows parsed, else fallback to single cell
                tout = io.BytesIO()
                with pd.ExcelWriter(tout, engine='xlsxwriter') as writer:
                    workbook = writer.book
                    ...
                    if rows:
                        headers = ['Scenario', 'Step Type', 'Step']
                        for c_idx, h in enumerate(headers):
                            worksheet.write(0, c_idx, h, header_fmt)
                        for r_idx, r in enumerate(rows, start=1):
                            worksheet.write(r_idx, 0, r['Scenario'], wrap)
                            worksheet.write(r_idx, 1, r['Step Type'], wrap)
                            worksheet.write(r_idx, 2, r['Step'], wrap)
                        worksheet.set_column(0, 0, 40, wrap)
                        worksheet.set_column(1, 1, 12, wrap)
                        worksheet.set_column(2, 2, 100, wrap)
                    else:
                        worksheet.write(0, 0, 'Test Cases (Markdown)', header_fmt)
                        worksheet.write(1, 0, tc_md, wrap)
                        worksheet.set_column(0, 0, 120, wrap)

                xls_tc_bytes = tout.getvalue()
            except Exception as e:
                st.warning(f"Excel export for test cases unavailable: {str(e)}")
            st.download_button(
                label="Download Test Cases (.xls)",
                data=xls_tc_bytes or b"",
                file_name="test_cases.xls",
                mime="application/vnd.ms-excel",
                key="dl_test_cases_xls",
                disabled=(xls_tc_bytes is None)
            )
