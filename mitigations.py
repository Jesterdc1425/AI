...
def create_mitigations_prompt(threats, framework: str = "STRIDE"):
    methodology_text = (
        "using the STRIDE threat modelling methodology"
        if (framework or "STRIDE").upper().startswith("STRIDE")
        else "aligned to the OWASP Top 10 categories"
    )
    prompt = f"""
Act as a cyber security expert with more than 20 years experience {methodology_text}. Your task is to provide potential mitigations for the threats identified in the threat model. It is very important that your responses are tailored to reflect the details of the threats.
